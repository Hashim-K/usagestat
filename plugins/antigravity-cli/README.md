...
usagestat reads non-secret context only from `~/.gemini/antigravity-cli/`. It does not read legacy Gemini OAuth files such as `~/.gemini/oauth_creds.json`.
...
The CLI and IDE appear to share Google platform/model quota, but usagestat tracks Antigravity CLI separately so the auth path and implementation remain clear.
