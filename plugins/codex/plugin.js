(function () {
  ...
  function readEnvUrl(ctx, name, fallback) {
    try {
  ...
  function refreshToken(ctx, authState) {
    ...
    try {
      const resp = ctx.util.request({
        method: "POST",
        url: readEnvUrl(ctx, "CODEX_REFRESH_URL", REFRESH_URL),
  ...
  function fetchUsage(ctx, accessToken, accountId) {
    ...
    return ctx.util.request({
      method: "GET",
      url: readEnvUrl(ctx, "CODEX_USAGE_URL", USAGE_URL),
      headers,
      timeoutMs: 10000,
    })
  }
  ...
  function probeWithAuthState(ctx, initialAuthState) {
    ...
    if (auth.tokens && auth.tokens.access_token) {
      ...
      try {
        ...
        ctx.host.log.warn("local Codex token usage failed: " + String(e))
      }
